...
// Setup Instructions
//
// 1) Copy the randomWeather.sqf file into your root mission folder.
//
...
weatherTemplates = [
        ...
        ["Light Fog",[0,2,5,6],[0.4,0,[0.2,0.01,20],0,0]],
        ["Medium Fog",[5,6,7],[0.4,0,[0.4,0.005,20],0,0]],
        ["Dense Fog",[6],[0.5,0,[0.5,0.0025,30],0,0]]
	];
...
// Check if there is no ParamsArray, and pick random if so, otherwise pick from paramsArray.
	...
	switch (initialWeatherParam) do {
		...
		case 3: {rw2_Current_Weather = 2;};  										// Light Rain
		case 4: {rw2_Current_Weather = 3;};  				 						// Medium Rain
		case 5: {rw2_Current_Weather = 4;};  				 						// Rainstorm
		case 6: {rw2_Current_Weather = 5;};  				 						// Light Fog
		case 7: {rw2_Current_Weather = 6;};  				 						// Medium Fog
		case 8: {rw2_Current_Weather = 7;};  				 						// Dense Fog
		case 9: {rw2_Current_Weather = floor(random(count(weatherTemplates)));};	// Random
		default {rw2_Current_Weather = 0;};
	};
