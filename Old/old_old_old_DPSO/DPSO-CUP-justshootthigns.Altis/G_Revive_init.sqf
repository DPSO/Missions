...
G_Revive_AI_Incapacitated = [WEST]; //Array of sides of AI that will utilize revive system
G_Revive_Unit_Exclusion = []; //Array of variable names of units to exclude from the revive system
G_Revive_bleedoutTime = -1; //Amount of time (in seconds) unit is available to be revived, before being forced to respawn. If -1, no time limit.
...
G_Revive_addonRadio_muteTransmit = true; //Mute radio transmissions in addons ACRE2 or TFAR while incapacitated. true = enabled, false = disabled.
...
G_Respawn_Time = 0; //Amount of time (in seconds) dead unit must wait before being able to respawn (overrides description.ext setting)
